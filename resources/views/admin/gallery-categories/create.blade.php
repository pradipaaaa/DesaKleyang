...
@section('content')
<form action="{{ route('admin.gallery-categories.store') }}" method="POST" class="max-w-xl space-y-5 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
    @csrf
    @include('admin.gallery-categories.partials.form', ['galleryCategory' => null])
